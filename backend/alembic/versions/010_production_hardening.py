...
def upgrade() -> None:
    ...
    op.add_column('problems', sa.Column('is_active', sa.Boolean(), nullable=False, server_default='true'))
    ...
    op.execute("UPDATE problems SET is_active = false, category = 'Algorithms' WHERE category = 'INACTIVE'")


def downgrade() -> None:
    op.execute("UPDATE problems SET category = 'INACTIVE' WHERE is_active = false")
